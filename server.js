...
db.then(function (db) {
  ...
  const convertToDate = (formatted) => {
    let extract_date = formatted.split(/[-/]/g)
  ...
  const join_logs = (user, results,rest=null) => {
    let logs = []
  ...
  app.get("/api/exercise/log", (req, res) => {
    ...
    if (/^[0-9A-Z]+$/gi.test(req.query.userId)) { //determine if id is in right format
      User.findById(req.query.userId, 'username _id', (err, user) => {
        if (user !== null) {
          let logQuery = Exercise.find({
            'userId': req.query.userId
          })
          if (req.query.from !== undefined || /\d{4}[-/]\d{2}[-/]\d{2}/.test(req.query.from) === true) {
            logQuery.where('date').gte(convertToDate(req.query.from))
            if (req.query.to !== undefined || /\d{4}[-/]\d{2}[-/]\d{2}/.test(req.query.to) === true) {
              logQuery.where('date').lte(convertToDate(req.query.to))
            }
          }
          if (req.query.limit !== undefined || /\d+/.test(req.query.limit) === true) {
            logQuery.limit(parseInt(req.query.limit))
          }
          let additional_info = logQuery.getQuery()
          logQuery.exec((err, logs) => {
            res.json(join_logs(user, logs,additional_info))
          })
      ...
      //User.findById(req.query.userId, 'username _id', (err, user) => {
      /*
      if (err || user === null) res.json("unknown userId")
      if (req.query.from === undefined || /\d{4}-\d{2}-\d{2}/.test(req.query.from) === false) {
        ...
        Exercise.where({
          userId: req.query.userId
        }).limit(parseInt(req.query.limit)).exec((err, results) => {
          console.log("from was not provided")
          res.json(join_logs(user, results));
        })
        ...
        if (req.query.limit === undefined || /\d+/.test(req.query.limit) === false) {
          Exercise.where({
              ...
              date: {
                $gte: convertToDate(req.query.from),
                $lte: to
              }
            })
            .exec((err, results) => {
